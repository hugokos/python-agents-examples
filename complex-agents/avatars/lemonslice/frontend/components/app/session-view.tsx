...
export const SessionView = ({
  ...
  // Get agent participant and read mode from attributes
  const agent = useRemoteParticipant({ kind: ParticipantKind.AGENT });
  const { attributes } = useParticipantAttributes({ participant: agent });
  const mode = attributes?.mode as 'roleplay' | 'coaching' | undefined;
  ...
  return (
    <section className="bg-background relative z-10 h-svh w-svw overflow-hidden" {...props}>
      ...
      />
      {/* Tile layout with mode */}
      <TileLayout chatOpen={chatOpen} mode={mode}>
        <SessionTimer startTime={sessionStartTime} duration={180} />
        <CoachingButton disabled={!session.isConnected || mode === 'coaching'} />
      </TileLayout>
